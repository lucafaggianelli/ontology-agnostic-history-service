...
class Query(Transaction):
    ...
    def rdf_query(self, triples):
        ...
        self._check_error(response)
        if "results" in response:
            results_list = parse_M3RDF(response["results"])
            return results_list
        else:
            raise SIBError(M3_SIB_ERROR)
...
class sparqlSubscribeHandler(threading.Thread):
    def __init__(self, node_id, tr_id, connector,
                 msg_handler):
        threading.Thread.__init__(self)
        self.node_id = node_id
    ...
    def run(self):
        while True:
            ...
            if msg["transaction_type"] == "SUBSCRIBE":
                if msg["message_type"] == "INDICATION":
                    added_list = parse_sparql(msg["new_results"])
                    removed_list = parse_sparql(msg["obsolete_results"])
                    self.msg_handler.handle(added_list, removed_list)
    ...
    def endElement(self, name):
        if name == "subject":
            ...
            if self.literal:
                ...
                o = URI(self.object)
            self.results_list.append(Triple(s, p, o))
            self.literal = False
            return
