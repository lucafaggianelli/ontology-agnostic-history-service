    ...
    def _encode(self, triples):
        ...
        for t, stype, otype in triples:
            ...
            if isinstance(self, Remove):
                if s == None or s == "sib:any"or s=="http://www.nokia.com/NRC/M3/sib#any":
                    s = "http://www.nokia.com/NRC/M3/sib#any"
                    stype = 'URI'

                if p == None or p == "sib:any":
                    p = "http://www.nokia.com/NRC/M3/sib#any"

                if o == None or o == "sib:any"or o=="http://www.nokia.com/NRC/M3/sib#any":
	    ...
	    if str(otype.lower())=="literal" :
            	tmp.extend([  '<object type = "%s"><![CDATA[%s]]></object>'%("literal", o)])
	    else :
		tmp.extend([  '<object type = "%s">%s</object>'%("uri", o)])
    ...
    def send(self, payload, pl_type = "python",
        ...
        """Accepted format for payload is determined by the parameter pl_type.
           ...
           - pl_type != 'python': payload is given in encoded form,
             ...
             8to the actual URIs stored in the SIB (works for rdf-m3
             encoding only)
        """
    ...
    def _encode(self, triples):
	    ...
	    # print "T=",t
            ...
            if len(t) == 3:
                ...
                triple, obj_type = t
            s, p, o = triple
            if s == None or s == "sib:any":
                s = "http://www.nokia.com/NRC/M3/sib#any"
            if p == None or p == "sib:any":
                p = "http://www.nokia.com/NRC/M3/sib#any"
            if o == None or o == "sib:any":
                o = "http://www.nokia.com/NRC/M3/sib#any"
                obj_type = 'URI'
            if o == 'sib:any' :
		o = "http://www.nokia.com/NRC/M3/sib#any"
		obj_type = 'URI'
	    elif o == "http://www.nokia.com/NRC/M3/sib#any":
                obj_type = 'URI'
...
class TCPConnector(Connector):
    ...
    def receive(self):
        ...
        if msg_end_index != -1:
            ...
            print msg
	    msg_unicoode=u""
	    msg_unicoode=unicode(msg.encode('string_escape'))
	    msg_ascii=msg_unicoode.encode('ascii','xmlcharrefreplace')
